...
ggsave("Qn1c.png")
 ...
 gdp.air.reg <- lm(Growth.Log.GDP~Mean.Annual.Exposure.PM2.5.2013, data = df.gdp.air1)
...
ggsave("Qn2.png")
 ...
 adr.reg.2 <- lm(Log.GDP.2013~Age.Dependency.Ratio+Old.Age.Dependency.Ratio, data = df.gdp.air.manu.adr.oadr)
...
```{r dataset tariff, echo=FALSE}
 ...
 #Identify Tariff with either Reporter's average tax >0 
...
dis.tariff.0 <- ggplot(tariff0, aes(x = Product.Num, y = AVDuty.average)) +
      ...
      theme_minimal() +  
      theme(plot.title = element_text(hjust = 0.5, face = "bold")) +
      labs(title = "Distribution of Duty Tax in Malaysia",
           x = "\nProduct Code",
           y = "Average Duty\n",
           colour = "Product Type",
...
#Mean of Malaysia Tariff
 mean.malay <- mean(tariff0$AVDuty.average, na.rm = "TRUE")
