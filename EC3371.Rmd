...
```{r age dependncy ration, include=FALSE}
 ...
 df.gdp.air.manu.adr <- merge.data.frame(df.gdp.air.manu, df.adr.13, by = "Country.Name")


```

```{r adr plot, echo=FALSE}

 ggplot(df.gdp.air.manu.adr, aes(x=Age.Dependency.Ratio, y= Log.GDP.2013)) +
    geom_point(shape=19) +   
    geom_smooth(method = loess, method.args = list(family = "symmetric")) + 
      ggtitle("Relationship between Log GDP and Age Dependency Ratio") + 
